...
impl<S: SecurityPlugin> EcuManager<S> {
    ...
    fn map_dop_from_uds(
        ...
        match dop.variant()? {
            ...
            datatypes::DataOperationVariant::Dtc(dtc_dop) => {
                tracing::debug!("map_dop_from_uds: Found DTC-DOP variant, calling map_dtc_dop_from_uds");
    ...
    fn map_dtc_dop_from_uds(
        ...
        tracing::debug!("map_dtc_dop_from_uds: byte_pos={}, bit_pos={}", 
            param.byte_position(), param.bit_position());
        
        let coded_type: datatypes::DiagCodedType = dtc_dop.diag_coded_type()?;

        let payload_data = uds_payload.data()?;
        tracing::debug!("map_dtc_dop_from_uds: payload={:02X?}", payload_data);
        ...
        tracing::debug!("map_dtc_dop_from_uds: decoded dtc_value={:02X?}", dtc_value);

        let code: u32 = DiagDataValue::new(coded_type.base_datatype(), &dtc_value)?.try_into()?;

        tracing::debug!("map_dtc_dop_from_uds: code={:X}", code);

        let available_dtcs: Vec<u32> = dtc_dop
            .dtcs()
            .map(|dtcs| dtcs.iter().map(|dtc| dtc.trouble_code()).collect())
            .unwrap_or_default();
        tracing::debug!("map_dtc_dop_from_uds: available DTCs={:X?}", available_dtcs);
